...
local function hasHarness()
    ...
    if GetResourceState('qb-smallresources') == 'started' then
        ...
        if ok then
            hasHarnessState = result == true
        end
    end

    if not hasHarnessState and LocalPlayer and LocalPlayer.state then
        hasHarnessState = LocalPlayer.state.harness == true
            or LocalPlayer.state.hasHarness == true
            or LocalPlayer.state.seatbeltHarness == true
    end

    harness = hasHarnessState
end
...
local function readFuel(vehicle)
    ...
    local fuel = nil
    local fuelScript = Config.FuelScript or 'LegacyFuel'

    if fuelScript and GetResourceState(fuelScript) == 'started' then
        local ok, result = pcall(function()
            return exports[fuelScript]:GetFuel(vehicle)
        end)
    ...
    if fuel == nil then
        ...
        if entity and entity.state then
            local stateFuel = entity.state.fuel
    ...
    while true do
        if LocalPlayer.state.isLoggedIn then
            ...
            if IsPedInAnyVehicle(ped, false) and not IsThisModelABicycle(GetEntityModel(GetVehiclePedIsIn(ped, false))) and not isElectric(GetVehiclePedIsIn(ped, false)) then
                if readFuel(GetVehiclePedIsIn(ped, false)) <= 20 then
